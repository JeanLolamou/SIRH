@extends('templates/default')
  ...
  <!-- <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.0/js/bootstrap.min.js"></script> -->
         ...
         <div class="row">
        <div class="col-lg-12">
          <h3 class="page-header"><i class="fa fa-building-o"></i> DEMANDES DE SALLE</h3>
          <ol class="breadcrumb">
            <li><i class="fa fa-home"></i><a href="{{route('home')}}">Accueil</a></li>
            <li><i class="fa fa-building-o"></i>Demande de salle</li>                
          </ol>
        </div>
      </div>
     <div class="row">
        ...
        <div class="col-md-5 col-sm-5 col-xs-6 col-xxs-12">
          <div class="smallstat red-bg">
            <a href="{{route('Reunion.create')}}"><i class="fa fa-plus white-bg"></i>

            <span class="title">Nouvelle demande de salle</span></a>
          </div><!--/.smallstat-->
        </div><!--/.col-->
        ...
        <div class="col-lg-12">
          <div class="panel panel-default">
            <div class="panel-heading">
              <h2><i class="fa fa-building-o red"></i><span class="break"></span><strong>Liste des demandes</strong></h2>
 ...
 <input class = "form-control" id = "demo" type = "text" placeholder = "Seach here,..">
         ...
         <table class = "table table-bordered table-striped">
            ...
            <tbody id = "test">
              @foreach ($reunion as $reunion)
                  <tr>
                    <td>{{nom_user($reunion->id_user)}}</td>
                    <td>{{$reunion->libelle}}</td>
                    <td>{{nom_salle($reunion->id_salle)}}</td>
                    <td>{{$reunion->date}}</td>
                    <td>{{(new DateTime($reunion->heur_debut))->format("H:s")}}</td>
                    <td>{{(new DateTime($reunion->heur_fin))->format("H:s")}}</td>
                  <td>
                    <?php if ($reunion->statut==0): ?>
                      <span class="label label-warning">En Cours</span>
                    <?php elseif ($reunion->statut==1): ?>
                      <span class="label label-info" >Validée</span>
                    <?php elseif ($reunion->statut==2): ?>
                  ...
                  <td>
                     <?php if ($reunion->statut==0): ?>
               <a data-toggle="modal" data-target="#valide{{$reunion->id}}" class="btn btn-primary"  href="#" title="Valider la demande" data-rel="tooltip">
                      <i class="fa fa-check"></i>                                            
                    </a>
                     <a data-toggle="modal" data-target="#annule{{$reunion->id}}" class="btn btn-danger"  href="#" title="Annuler la demande." data-rel="tooltip">
                      <i class="fa fa-times"></i>                                            
                    </a>

                      @if(Auth::user()->id==$reunion->id_user)
                    <a class="btn btn-info" href="{{ route ('Reunion.edit', $reunion->id)}}">
                      <i class="fa fa-edit " title="Modifier." data-rel="tooltip"></i>                                            
                    </a>
                    <a data-toggle="modal" data-target="#myModal{{$reunion->id}}" type="button" class="btn btn-default" href="#" title="Supprimer." data-rel="tooltip">
            ...
            <?php endif ?>
                    ...
                    <a class="btn btn-success" href="{{ route ('Reunion.show', $reunion->id)}}" title="Voir plus de details." data-rel="tooltip">
                      <i class="fa fa-search-plus "></i>                                            
                    </a>

                    <!-- Validation -->

                    <div class="modal fade" id="valide{{$reunion->id}}">
  ...
  <!-- Annulation -->
                    ...
                    <div class="modal fade" id="annule{{$reunion->id}}">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
